		...
		for ( mIter = vIter->begin(); mIter != vIter->end(); )
		{
			if ( mIter->second.size() != 0 )
			{
				...
				for ( vArrIter; vArrIter != mIter->second.end(); )
				{
					...
					//벡터의 요소를 지우는 erase 함수를 쓰려면 반복자를 넣어줘야 하는데
					//erase 함수를 쓰면 넣어준 반복자의 다음값을 리턴해준다.
...
void effectManager::play( string effectName, int x, int y )
{
	...
	for ( vIter = _vTotalEffects.begin(); vIter != _vTotalEffects.end(); ++vIter )
	{
		...
		for ( mIter = vIter->begin(); mIter != vIter->end(); ++mIter )
		{
			//이펙트 이름과 비교해서 같이 않으면 다음 맵으로 넘어감
			if ( !( mIter->first == effectName ) ) break;

			//여기까지 내려왔다는 것은 찾았다는 거지?
			//이펙트 키와 일치하면 이펙트를 실행시킨다.
			iterEffects vArrIter;
			for ( vArrIter = mIter->second.begin(); vArrIter != mIter->second.end(); ++vArrIter )
			{
				//이펙트 터트리자
				//미리 이펙트가 활성화 되어 있으면 다음걸 스타트
